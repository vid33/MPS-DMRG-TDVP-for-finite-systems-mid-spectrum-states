function [ Fr_1S, Fr_2S ] = calculate_Fr( A, rhor, H1S, H2S, N )
    ...
    for kk=(N-1):-1:1
        if kk>1
            tmp1 = Contract({ A{kk}, conj(A{kk}), rhor{kk} }, {[-2, -1, 1], [-4, -3, 2], [1, 2]});
            F1S_cell{kk} = Contract({H1S{kk}, tmp1}, {[1, 2], [1, -1, 2, -2] });
        elseif kk == 1
            tmp1 = Contract({ A{kk}, conj(A{kk}), rhor{kk} }, {[-1, 1], [ -2, 2], [1, 2]});
            F1S_cell{kk} = Contract({H1S{kk}, tmp1}, {[1, 2], [1, 2] });
        end
    end
    
    for kk=(N-2):-1:1
        tmp1 = Contract({A{kk+1}, conj(A{kk+1}), rhor{kk+1}}, {[-2, -1, 1], [-4, -3, 2], [1,2]});
