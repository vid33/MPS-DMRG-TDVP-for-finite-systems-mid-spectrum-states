function [ X ] = calculate_X(A, rhol, rhor, Vl, Fl, H1S, H2S )
    ...
    for kk=1:N-1    
        if kk==1
            ...
            tmp1 = Contract({rhol{kk-1}^(1/2), A{kk}, conj(Vl{kk})}, {[1, 2], [1, -1, -2], [2, -3, -4]});
        end
    ...
    for kk=1:(N-1)
        ...
        if kk==N-1
            tmp3 = Contract({tmp2, tmpA, A{kk+1}}, {[1, 2, 3, 4], [3, 4, -1], [1, 2]});
        else
            tmpB = Contract({A{kk+1}, rhor{kk+1}^(1/2)}, {[-1, -2, 1], [1, -3]});
            tmp3 = Contract({tmp2, tmpB, tmpA}, {[1, 2, 3, 4], [1, 2, -2], [3, 4, -1]});    
        end
    ...
    for kk=1:N
        if kk==1
            ...
            tmp1 = Contract({rhol{kk-1}^(1/2), A{kk}, rhor{kk}^(1/2)}, {[1, -1], [1, -2, 2], [2, -3]});
        end
        if kk==1
    ...
    for kk=1:N-1
        ...
        if kk==N-1
             ...
             X{kk+1} = X{kk+1} + Contract({tmp1, tmpA}, {[1, 2], [1, 2, -1]});
        else
            tmp1 = Contract({Fl{kk}, A{kk+1}, rhor{kk+1}^(1/2)}, {[1, -1], [1, -2, 2], [2, -3]});
            X{kk+1} = X{kk+1} + Contract({tmp1, tmpA}, {[1, 2, -2], [1, 2, -1]});
        end
%kk
